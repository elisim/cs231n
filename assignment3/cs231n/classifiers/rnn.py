...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        h0, h0_cache = affine_forward(features, W_proj, b_proj) # (1)
        embedding_out, embedding_cache = word_embedding_forward(captions_in, W_embed) # (2)
        # ====== (3)
        if self.cell_type == "rnn": 
            h_out, forward_cache = rnn_forward(embedding_out, h0, Wx, Wh, b)
        else: # lstm
            h_out, forward_cache = lstm_forward(embedding_out, h0, Wx, Wh, b)
        # ====== (3)
        vout, v_cache = temporal_affine_forward(h_out, W_vocab, b_vocab) # (4)
        loss, dx = temporal_softmax_loss(vout, captions_out , mask) # (5)
    ...
    def sample(self, features, max_length=30):
        ...
        prev_c = np.zeros(prev_h.shape) # for lstm
