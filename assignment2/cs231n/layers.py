...
def affine_forward(x, w, b):
    ...
    # affine => f(x) = Wx+b 
    N = x.shape[0]
    out = np.dot(x.reshape(N,-1), w) + b # NxD * D*M = NxM
...
def relu_backward(dout, cache):
    ...
    ###########################################################################
    # dx = 0 for < 0 and 1 otherwise
    
    d_relu = (x > 0) * 1
    dx = d_relu * dout # element-wise
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_bn = (x - sample_mean)/np.sqrt(sample_var + eps)
        out = gamma * x_bn + beta
    
        cache = (gamma, x, sample_mean, sample_var, eps, x_bn)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        ...
        # See shift & scale explanation here: https://github.com/alexgkendall/caffe-segnet/issues/109
        gamma_scaled = gamma / np.sqrt(running_var  + eps)
        beta_scaled = beta - running_mean * gamma_scaled
        out =  gamma_scaled * x + beta_scaled
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_ans, W_ans = int(1 + (H + 2 * pad - HH) / stride), int(1 + (W + 2 * pad - WW) / stride)
    out = np.random.randn(N,F, H_ans,W_ans)
    x_pad = np.pad(x, ((0,0),(0,0),(1,1),(1,1)), 'constant', constant_values=((0,0),(0,0),(0,0),(0,0)))

    for ni in range(N):
        for fi in range(F):
            for xi in range(H_ans):
                for yi in range(W_ans):
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_ans, W_ans = int(1 + (H - pool_height) / stride), int(1 + (W - pool_width) / stride)
    out = np.random.randn(x.shape[0], x.shape[1], H_ans, W_ans)
...
def max_pool_backward_naive(dout, cache):
    ...
    N, C, H, W = x.shape
    H_ans, W_ans = dout.shape[2], dout.shape[3]
    dx = np.zeros_like(x)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    x_ = x.transpose(0,2,3,1).reshape(-1,C) # permute to N,H,W, C and than to N*H*W , C times
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    dout_ = dout.transpose(0, 2, 3, 1).reshape(-1, C) # same idea as forward
